...
**Open a draft PR immediately when starting a worktree — don't wait until the work is finished.** As soon as the worktree has its first commit (an initial `git commit --allow-empty` stub is fine if you want the PR open before any real change), push and `gh pr create --draft`. Two reasons: (a) the work is visible and reviewable from the first commit, and (b) — decisive in repos that **auto-arm auto-merge on every _non-draft_ PR** (e.g. `fnioc/std`) — a *draft* is the only thing that holds in-progress work from landing before it's ready. Keep committing to the same branch; the commits flow into the open draft. Then `gh pr ready <n>` **only** when the work is finished *and* I've had any review I asked for — marking-ready is what releases it to the (auto-armed or manual) merge. This supersedes "open the PR with `--auto` at the very end" in the flows below: open the **draft** up front, work, mark ready at the end.
...
**2. Trivial edit in the parent session (≤~10 LOC).** `EnterWorktree(name: "chore-foo-bar")` → commit → push → `gh pr create --draft` (up front, per the rule above) → edit/commit as needed → `gh pr ready <n>` once done + reviewed → `ExitWorktree(action: "keep")` (or `"remove"` if already merged). See §"Entering / exiting worktrees" above for tool mechanics; `~/.claude/CLAUDE.md`§"All code edits happen in a worktree" for when this carve-out applies.
...
> **Give a sync 15 minutes before calling it failed.** Observed lag from merge to the last section landing: **~13 minutes**. The app creates no check run in any version, so a sync still in progress is indistinguishable from one that died. Reading the state early and concluding sections were skipped produces a confident, wrong root cause.
...
- **It is not a reconciler, and its trigger set is narrow.** It syncs *only* on a push to the default branch whose commit added or modified `settings.yml`, on `repository.created`, and on `repository.edited` when the default branch changed. Nothing else — a `label` event does not trigger it, despite the installation subscribing to `label`. Arbitrary drift is never corrected: change a setting by hand and it simply persists. To force a sync you must push a change to the file itself; a comment-only edit is enough.
