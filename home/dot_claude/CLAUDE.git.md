...
The policy — "code edits happen in a worktree subagent," when it applies, opt-out repos, and the named dispatch shapes — lives in `~/.claude/CLAUDE.md`'s "All code edits happen in a worktree subagent" section. This file covers the git mechanics for the two shapes named there.

**PR-bound subagent.** Parent creates the worktree with a chosen branch name, then dispatches the subagent **without** `isolation: "worktree"`, prompting it to `cd` into the worktree as its first action:
...
**Research-only subagent.** `Agent(isolation: "worktree", ...)`. Fires the hook; produces an `agent-<id>` workspace name. The worktree gets cleaned up automatically — auto-removed if the agent makes no changes; manually removed by the parent otherwise.
