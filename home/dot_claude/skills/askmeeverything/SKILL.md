---
name: askmeeverything
description: Works through the whole /askme queue in one pass — each open item presented one at a time in /explain's format, auto-advancing to the next as soon as the current one is answered, no "next" required. Triggered by /askmeeverything (an optional argument narrows scope the same way it does for /askme).
---

# Ask Me Everything — clear the /askme queue
...
invocations while an item sits unanswered. /askmeeverything is a single invocation covering the
whole queue: the moment the current item is answered (or explicitly skipped/deferred), immediately
present the next item in the same turn. Don't stop and wait for "next" and don't require the
owner to re-invoke /askmeeverything for each item.
...
These are the two things that keep getting dropped when presenting under /askmeeverything specifically, so
restate them here even though /askme already states them:

- **One at a time.** Never batch multiple open items into a single message, even though
  /askmeeverything is one continuous invocation. Present item 1, get the answer, present item 2 as a *new*
...
ties, age breaks remaining ties; an argument passed to /askmeeverything narrows scope the same way
an argument to /askme does.
